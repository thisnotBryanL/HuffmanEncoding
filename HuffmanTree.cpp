...
void HuffmanTree::create_node_array()
{
    for (int i = 0; i < 128; i++)
    {
...
void HuffmanTree::calculate_huffman_codes()
{
    traverse(root, "");
}

void HuffmanTree::coding_save()
{
    in_file.open(in_file_name, ios::in);
    ...
    {//get all characters and their HuffmanTree codes for output
        ...
        s += '1';																			//'1' indicates the start of HuffmanTree code
...
void HuffmanTree::recreate_huffman_tree()
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        int j = 0;
        while (h_code_s[j] == '0')
        {//delete the added '000����1' to get the real HuffmanTree code
            j++;
        }
...
void HuffmanTree::decoding_save()
{
    in_file.open(in_file_name, ios::in | ios::binary);
    out_file.open(out_file_name, ios::out);

    unsigned char size;																		//get the size of HuffmanTree tree
    in_file.read(reinterpret_cast<char*>(&size), 1);

    in_file.seekg(-1,ios::end);
    char magicNumber;
    in_file.read(&magicNumber,1);
    if(magicNumber != '6'){
